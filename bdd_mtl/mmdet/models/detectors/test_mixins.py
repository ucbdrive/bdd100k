...
class TrackTestMixin(object):
    ...
    def setup_tracklets(self,
        ...
        track_bboxes = det_bboxes[valid_idx]
        track_labels = det_labels[valid_idx]
        if rescale:
            scale_factor = img_meta[0]['scale_factor']
            _track_bboxes = track_bboxes[:, :4] * scale_factor
        else:
            _track_bboxes = track_bboxes[:, :4]
        track_rois = bbox2roi([_track_bboxes])
        track_feats = self.asso_track_roi_extractor(
            x[:self.asso_track_roi_extractor.num_inputs], track_rois)
        track_embedding = self.asso_track_head.get_embeds(track_feats)
        ids = torch.arange(track_bboxes.shape[0]).to(track_embedding.device)
        vanish_frames = torch.zeros_like(track_labels)
        self.update(
            type='inplace',
            embeddings=track_embedding,
            tracklet_scores=track_bboxes[:, 4].detach().clone(),
            bboxes=track_bboxes.detach().clone(),
            labels=track_labels,
    ...
    def simple_test_asso_track(self,
                               x,
                               det_bboxes,
                               det_labels,
                               img_meta,
    ...
    def assign_ids(self, prop_bboxes, asso_probs, det_bboxes, det_labels):
        ...
        self.update(
            type='contact',
            embeddings=self.cur_embs[valid_idxs, :],
            tracklet_scores=new_track_bboxes[:, -1].detach().clone(),
            bboxes=new_track_bboxes.detach().clone(),
            labels=new_track_labels,
            vanish_frames=vanish_frames)
        return ids, det_bboxes, det_labels

    def update_tracklets(self, x, ids, det_bboxes, det_labels):
        """
        ...
        unmatch_idx = torch.nonzero(ids < 0).squeeze(1).tolist()
        instance_ids = ids[match_idx].tolist()
        # update embedding
        m = self.test_cfg.track.embed_momentum
        self.embeddings[instance_ids] = (
            1 - m) * self.embeddings[instance_ids].detach().clone(
            ) + m * self.cur_embs[match_idx, :].detach().clone()
        ...
        # assert for label
        assert torch.equal(self.labels[instance_ids], det_labels[match_idx])
        ...
        # update det_bboxes
        # TODO better way to update det_bboxes confidence
        if self.test_cfg.track.update_score:
            det_bboxes[match_idx, -1] += self.tracklet_scores[instance_ids]
            det_bboxes[match_idx, -1] /= 2
            det_bboxes[unmatch_idx, -1] /= 2
        # update bboxes
